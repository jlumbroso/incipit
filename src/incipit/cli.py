...
def main(audit, count, pages, systems, height_threshold, width_threshold, output, verbose, input):
    ...
    if count and output is None and systems is None:
        ...
        for i in range(offset, 252):
        #for i in range(offset, 559):
          ...
          if( len(systems) - systemNumberInPDFList[i-offset] != 0 ):
            print(str(len(systems)) + "\t" + str(len(systems) - systemNumberInPDFList[i-offset]) + "\t" + input )
          #else:
          #  print("matched " + str(i) + ".pdf")
          totalError += abs(len(systems) - systemNumberInPDFList[i-offset])
